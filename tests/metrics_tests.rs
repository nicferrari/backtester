...
fn sma_metrics_tests() {
    ...
    assert_eq!(sma_bt.metrics.max_pl.unwrap(), 42.78189783821916);
    assert_eq!(sma_bt.metrics.min_pl.unwrap(), -17.225731871090343);
    assert_eq!(sma_bt.metrics.average_pl.unwrap(), 0.4872716030712669);
    assert_eq!(sma_bt.metrics.win_rate.unwrap(), 0.32222222222222224);
    assert_eq!(sma_bt.metrics.max_drawd.unwrap(), 0.5376575874681033);
    assert_eq!(
        sma_bt.metrics.sharpe.unwrap() * 252f64.sqrt(),
        0.2065342502984144
    ); //sharpe ratio considers rf=0*/
    assert_eq!(sma_bt.metrics.avg_duration.unwrap(), 10.022222222222222);
}
