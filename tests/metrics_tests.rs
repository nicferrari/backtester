...
fn buynhold_metrics_tests(){
    ...
    assert_eq!(buynhold_bt.metrics.sharpe.unwrap()*252f64.sqrt(),1.0083070085617758);//sharpe ratio needs to be annualized (and is on working days returns but annualized by days). metrics should already have the annualization?
}
#[test]
fn sma_metrics_tests(){
    ...
    assert_eq!(sma_bt.metrics.sharpe.unwrap()*252f64.sqrt(),0.7009220717545875);//sharpe ratio considers rf=0
}
