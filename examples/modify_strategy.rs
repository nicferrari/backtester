...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let sma_cross_inverted_bt = Backtest::new(sma_cross_inverted, 100_000.);
    report_horizontal(&[&sma_cross_inverted_bt]);
    //let's try a long only
    let sma_cross_long = sma_cross_strategy.clone().long_only();
    let sma_cross_long_bt = Backtest::new(sma_cross_long, 100_000.);
    report_horizontal(&[&sma_cross_long_bt]);
