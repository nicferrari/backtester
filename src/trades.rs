...
pub fn trade_indices_from_broker(broker: &Broker)-> TradeList {
    ...
    let indices = pairs.into_iter().enumerate().map(|(i,(a,b))| Trade {position:i,open_index:a,close_index:b}).collect();
    TradeList {indices}
}
...
impl Trade {
    ...
    fn calc_pl(&self, strategy: Strategy, cfg:Config) -> f64 {
        strategy.choices[self.open_index-1].sign() as f64 *(cfg.execution_time.to_quotes(strategy.data.clone(), self.close_index)/cfg.execution_time.to_quotes(strategy.data.clone(), self.open_index)/(1.+cfg.commission_rate).powi(2*strategy.choices[self.open_index - 1].sign() as i32)).ln()*100.
    }
...
impl TradeList {
    ...
    pub fn calculate_metrics(&self, metrics: &mut Metrics, strategy: Strategy){
        ...
        let max_pl = self.indices.iter().map(|t |t.calc_pl(strategy.clone(), cfg.clone())).reduce(f64::max).unwrap();
        let min_pl = self.indices.iter().map(|t|t.calc_pl(strategy.clone(), cfg.clone())).reduce(f64::min).unwrap();
        let win_rate = self.indices.iter().map(|t|t.calc_pl(strategy.clone(), cfg.clone())).filter(|&i|i>0.).count() as f64/self.indices.len() as f64 ;
        let average_pl = self.indices.iter().map(|t|t.calc_pl(strategy.clone(), cfg.clone())).sum::<f64>()/self.indices.len() as f64;
