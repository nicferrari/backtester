...
impl Broker{
    pub fn print_stats(&self){
        ...
        let max_drawdown = self.available.iter().map(|v| { if v > peak { peak = v; }(peak - v) / peak }).fold(0.0, |max_dd, dd| dd.max(max_dd));
...
pub fn calculate(strategy:&Strategy, initial_account:f64) ->Broker{
    ...
    for i in 0..status.len(){
        availables[i..].fill(positions[i] as f64*cfg.execution_time.to_quotes(strategy.data.clone(), i)+accounts[i]);
        if status[i]==Status::Executed{
            fees[i] = (positions[i] as f64).abs()*cfg.execution_time.to_quotes(strategy.data.clone(), i)*cfg.commission_rate;
            positions[i..].fill(((strategy.choices[i].sign() as f64)*(availables[i]-fees[i])/cfg.execution_time.to_quotes(strategy.data.clone(), i)/(1.+cfg.commission_rate)) as i32);
            invested[i..].fill(positions[i] as f64*cfg.execution_time.to_quotes(strategy.data.clone(), i));
        ...
        mtm[i]=positions[i] as f64 * cfg.execution_time.to_quotes(strategy.data.clone(), i) - invested[i];//todo! mtm is now calculated on execution_time: do on close?
