...
use crate::orders::Order::{BUY, NULL, SHORTSELL};
use crate::ta::{rsi, sma, Indicator};
use serde::Serialize;
use std::sync::Arc;
...
impl Strategy {
    ...
    pub fn invert(&self) -> Self {
        ...
        for (choices, inv_choices) in self.choices.iter().zip(inv_choices.iter_mut()) {
            if *choices == BUY {
                *inv_choices = SHORTSELL
            } else if *choices == SHORTSELL {
                *inv_choices = BUY
            };
        }
    ...
    pub fn skipfirst(&self) -> Self {
        ...
        for (i, window) in self.choices.windows(2).enumerate() {
            ...
            new_choices[i + 1] = if change_count < 2 { NULL } else { window[1] };
        }
        Strategy {
...
pub fn short_n_hold(quotes: Arc<Data>) -> Strategy {
    let length = quotes.datetime.len();
    let mut choices = vec![SHORTSELL; length];
...
pub fn sma_strategy(quotes: Arc<Data>, period: usize) -> Strategy {
    ...
    for ((ind, close), choice) in indicator
        .indicator
        .iter()
        .zip(&indicator.quotes.close)
        .zip(choices.iter_mut())
    {
        if *ind != -1. {
...
pub fn sma_cross(quotes: Arc<Data>, short_period: usize, long_period: usize) -> Strategy {
    if short_period >= long_period {
        panic!("Error: short SMA parameter should be shorter than long SMA parameter");
    }
    let sma_short = sma(&quotes, short_period);
    let sma_long = sma(&quotes, long_period);
    let ind_short = Indicator {
        indicator: sma_short,
        quotes: quotes.clone(),
    };
    let ind_long = Indicator {
        indicator: sma_long,
        quotes: quotes.clone(),
    };
    let length = ind_short.quotes.datetime.len();
    let mut choices = vec![NULL; length];
    for ((ind_long, ind_short), choices) in ind_long
        .indicator
        .iter()
        .zip(ind_short.indicator.iter())
        .zip(choices.iter_mut())
    {
        if *ind_long != -1. {
            *choices = if *ind_short > *ind_long {
                BUY
            } else {
                SHORTSELL
            }
