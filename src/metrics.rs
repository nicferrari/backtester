...
pub fn report_horizontal(backtests: &[&Backtest]) {
    for item in backtests {
        println!(
            ...
            item.strategy.data.datetime.first().unwrap().date_naive(),
            item.strategy.data.datetime.last().unwrap().date_naive(),
            (*item.strategy.data.datetime.last().unwrap()
                - item.strategy.data.datetime.first().unwrap())
            .num_days()
        );
...
pub fn report_vertical(backtests: &[&Backtest]) {
    ...
    print_field!(
        "Ticker",
        backtests,
        |i: &Backtest| i.metrics.ticker.clone(),
        |s| s,
        TAB
    );
    print_field!(
        "Strategy",
        backtests,
        |i: &Backtest| i.metrics.strategy_name.clone(),
        |s| s,
        TAB
    );
    print_field!(
    ...
    print_field!(
        "Exp time",
        backtests,
        |i: &Backtest| i.metrics.exposure_time,
        |s| format!("{:.2}%", s * 100.),
        TAB
    );
    print_field!(
        "Trades #",
        backtests,
        |i: &Backtest| i.metrics.trades_nr,
        |a| a,
        TAB
    );
    print_field!(
    ...
    print_field!(
        "Avg P&L",
        backtests,
        |i: &Backtest| i.metrics.average_pl,
        |s| format!("{:.2}%", s),
        TAB
    );
    print_field!(
    ...
    print_field!(
        "Avg dur (d)",
        backtests,
        |i: &Backtest| i.metrics.avg_duration,
        |s| format!("{:.2}", s),
        TAB
    );
    print_field!(
        "Max Drawdown",
        backtests,
        |i: &Backtest| i.metrics.max_drawd,
        |s| format!("{:.2}%", s * 100.),
        TAB
    );
    print_field!(
        "Sharpe r",
        backtests,
        |i: &Backtest| i.metrics.sharpe,
        |s| format!("{:.2}", s * 252f64.sqrt()),
        TAB
    );
    println!("{}", "_".repeat(100));
